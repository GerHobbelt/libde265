 ...
 */
void decoder_context::process_inter_layer_reference_picture_set(slice_segment_header* hdr)
{
  // The variable currLayerId is set equal to nuh_layer_id of the current picture
  int currLayerId = img->get_ID();
  ...
  // Get VPS
  std::shared_ptr<video_parameter_set> vps = current_vps;
  ...
  for( int i=0; i<hdr->NumActiveRefLayerPics; i++ ) {
    ...
    decoder_context *ctx_lower = get_multi_layer_decoder()->get_layer_dec(refLayerID);
    ...
    if (DPBIndex != -1) {
      ...
      derive_inter_layer_reference_picture(rlPic, refLayerID, i);
 ...
 */
void decoder_context::derive_inter_layer_reference_picture(de265_image* rlPic, int rLId, int ilRefPicIdx)
{
  // Get the pps, pps_ext and sps
  std::shared_ptr<pic_parameter_set> pps = current_pps;
  pps_multilayer_extension* pps_ext = &pps->pps_mult_ext;
  std::shared_ptr<seq_parameter_set> sps = current_sps;
  ...
  decoder_context *ctx_ref = get_multi_layer_decoder()->get_layer_dec(rLId);
  std::shared_ptr<seq_parameter_set> sps_ref = ctx_ref->current_sps;
  ...
  if (ilRefPic[ilRefPicIdx] == NULL) {
    ...
    ilRefPic[ilRefPicIdx]->alloc_image(PicWidthInSamplesCurrY, PicHeightInSamplesCurrY, c, sps, false, this, NULL, 0, 0, false, true);
  }
  ...
  if (equalPictureSizeAndOffsetFlag &&
    ...
    video_parameter_set_extension *vps_ext = &current_vps->vps_extension;
    int currLayerId = get_layer_id();
  ...
  *err = DE265_OK;
  ...
  if (hdr->first_slice_segment_in_pic_flag) {
    ...
    if (hdr->first_slice_segment_in_pic_flag) {
      ...
      if (layer_ID > 0) {
        process_inter_layer_reference_picture_set(hdr);
      }
