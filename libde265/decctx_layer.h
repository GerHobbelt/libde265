...
{
public:
  slice_unit(decoder_context_layer* decctx);
  ~slice_unit();
...
class decoder_context_layer : public base_context {
 public:
  decoder_context_layer();
  ~decoder_context_layer();
  ...
  */
  ...
  bool process_slice_segment_header(decoder_context_layer*, slice_segment_header*,
 ...
 private:
  ...
  void process_picture_order_count(decoder_context_layer* ctx, slice_segment_header* hdr);
  int generate_unavailable_reference_picture(decoder_context_layer* ctx, const seq_parameter_set* sps,
                                             int POC, bool longTerm);
  void process_reference_picture_set(decoder_context_layer* ctx, slice_segment_header* hdr);
  bool construct_reference_picture_lists(decoder_context_layer* ctx, slice_segment_header* hdr);
